...
##     species <- "Gray Catbird"    arr_master$species[1] ; rang <- "mig" ; mod <- "GAM" ; PP_YEAR <- 2002
doplot <- function(PP_YEAR, species,mod,rang) {
  ...
  hexgrid6 <- dggridR::dgconstruct(res = 6)
    if(dim(arr_f) [1] == 0){
      arr_f <- arr_master[which(arr_master$species == species),]
      arr_f <- arr_f[which(arr_f$per_ovr >= 0.05),]
      arr_f <- arr_f[which(is.na(arr_f$arr_GAM_mean) & is.na(arr_f$arr_IAR_mean)),]
      arr_f <- arr_f[!duplicated(arr_f$cell),]
      
      if(dim(arr_f) [1] == 0) {
        arr_f <- arr_master[which(arr_master$species == species),]
        arr_f <- arr_f[which(arr_f$per_ovr >= 0.05),]
        arr_f <- arr_f[!duplicated(arr_f$cell),]
        arr_f$arr_GAM_mean <- as.numeric(NA)
        arr_f$arr_IAR_mean <- as.numeric(NA)
      }
    }
    ...
    if(mod == "IAR") { 
      to_plt$arr_IAR_mean <- as.numeric(to_plt$arr_IAR_mean)
      ...
      to_plt$arr_GAM_mean <- as.numeric(to_plt$arr_GAM_mean)
      elp <- 
      pp +
      geom_polygon(data = to_plt, aes(x = long, 
                                      y = lat, group = group, 
