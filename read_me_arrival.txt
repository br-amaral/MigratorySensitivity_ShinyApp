...
posterior_mean - posterior mean of arrival estimate. These estiamtes were derived from data from eBird.
posterior_sd - posterior standard deviation arrival estimate. These estiamtes were derived from data from eBird.


DATA USE RECOMMENDATIONS
------------------------
It is recommended that if using IAR-derived estimates, that only species-cells-years with associated GAM data be used for downstream analyses. For those cases where IAR-derived estimates are available but GAM-derived estimates are not available, values have been interpolated beyond the spatial extent (for that species-year) of data used to fit the model and should therefore be used with caution.
